        ...
        <div className="relative mt-6 border bg-slate-100 rounded-md p-4">
            ...
            {!isCreating && (
                <div className={cn("text-sm mt-2",!initialData.chapters.length && 'text-slate-500 italic')}>
                    ...
                    <ChaptersList items={initialData.chapters || []} onReorder={onReorder} onEdit={onEdit}/>
                </div>
            )}
            {!isCreating && (
                <p className="text-xs text-muted-foreground mt-4">Drag And Drop To Reorder The Chapter</p>
            )}
        </div>
    )
}
